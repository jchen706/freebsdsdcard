//reference from FreeBSD Device Driver Chapter 13.
...
static int at45d_probe(device_t dev)
{
        device_set_desc(dev, "AT45 flash family");
        return (BUS_PROBE_SPECIFIC);
}

static int at45d_attach(device_t dev)
{
...
static int at45d_detach(device_t dev)
{
        return (EIO);
}

static int at45d_get_info(device_t dev, uint8_t *r)
{
...
static uint8_t at45d_get_status(device_t dev)
{
          struct spi_command cmd;
          uint8_t tx_buf[8], rx_buf[8];
...
static void at45d_wait_for_device_ready(device_t dev)
{
          while ((at45d_get_status(dev) & 0x80) == 0)
                  continue;
}

static void at45d_delayed_attach(void *arg)
{
